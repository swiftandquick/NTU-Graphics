package Section1Problem2_Alternative;
...
public class AWTAccumulator extends JFrame implements ActionListener {
	...
	JLabel row2Label = new JLabel("Accumulated sum is 0");
	...
	public void actionPerformed(ActionEvent event) {
		...
		/** Split the row2Label text into two substrings.  */
		String split[] = row2Label.getText().split(" is ");
		/** The second substring should contain the sum.  */
		String sumT = split[1];
		...
		/** Resets both integer and sum to 0.  Re-enable integer text field if it's disabled.  */
			...
			integer.setText("0");
			row2Label.setText("Accumulated sum is 0");
		}
	...
	public void setTextField(String rIntegerT, String rSumT) {
		...
		try {
			...
			row2Label.setText("Accumulated sum is " + Integer.toString(rSum));
		}
		catch (Exception e) {
			integer.setEnabled(false);
			row2Label.setText("Accumulated sum is ERROR");
		}
